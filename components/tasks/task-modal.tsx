...
export function TaskModal({ task, labels, members = [], currentUserId, today, dateFormat, onClose, onChanged, onDelete, onPrev, onNext }: {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] sm:p-4 md:py-[6vh]" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="task-dialog-title" tabIndex={-1} className="task-dialog mx-auto flex h-full max-h-[calc(100dvh-1rem-env(safe-area-inset-bottom))] w-full max-w-3xl flex-col overflow-hidden rounded-xl border bg-background shadow-xl md:max-h-[82vh]">
      <header className="flex items-center gap-1 border-b px-4 py-2">
        <a href={`/projects/${task.projectId}`} className="mr-auto text-sm text-muted-foreground hover:text-foreground">{projectName}</a>
        ...
        <DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon-xs" aria-label="Task actions"><Ellipsis className="size-3.5" /></Button>} /><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => void navigator.clipboard.writeText(`${location.origin}/projects/${task.projectId}?task=${task.id}`)}>Copy link to task</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onClick={() => { onDelete(task); onClose(); }}>Move to Trash</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
        <Button ref={closeButtonRef} variant="ghost" size="icon-xs" aria-label="Close" onClick={onClose}><X className="size-3.5" /></Button>
      </header>
...
function CommentRow({ comment, author, canEdit, onEdit, onDelete }: { comment: Comment; author?: string; canEdit: boolean; onEdit: (content: string) => void; onDelete: () => void }) {
  ...
  return <div className="group flex items-start gap-2 py-2"><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-medium text-primary">{author?.charAt(0).toUpperCase() ?? "?"}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="text-xs"><span className="font-medium">{author ?? "Member"}</span> <span className="text-muted-foreground">{new Date(comment.createdAt).toLocaleString()}</span></p>{canEdit && <span className="flex opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100"><Button variant="ghost" size="icon-xs" aria-label="Edit comment" onClick={() => setEditing(true)}><Pencil className="size-3.5" /></Button><Button variant="ghost" size="icon-xs" aria-label="Delete comment" onClick={onDelete}><Trash2 className="size-3.5" /></Button></span>}</div><p className="whitespace-pre-wrap text-base sm:text-sm">{comment.content}</p></div></div>;
}
...
function CommentInput({ avatar, onSubmit, onAttachment, uploading, fileInputRef }: { avatar: string; onSubmit: (content: string) => void; onAttachment: (file: File) => void; uploading: boolean; fileInputRef: RefObject<HTMLInputElement | null> }) {
  ...
  return <div className="mt-2 flex items-center gap-2"><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-medium text-primary">{avatar}</span><form onSubmit={submit} className="flex flex-1 items-center gap-1 rounded-full border px-3 py-1.5 focus-within:ring-1 focus-within:ring-ring"><input className="min-w-0 flex-1 bg-transparent text-base outline-none sm:text-sm" placeholder="Comment" value={content} onChange={(e) => setContent(e.target.value)} /><Button type="button" variant="ghost" size="icon-xs" aria-label="Add attachment" disabled={uploading} onClick={() => fileInputRef.current?.click()}><Paperclip className="size-3.5" /></Button><input ref={fileInputRef} type="file" className="hidden" disabled={uploading} onChange={(e) => { const file = e.target.files?.[0]; if (file) onAttachment(file); }} /></form></div>;
}
