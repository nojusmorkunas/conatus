# Maintainer release plan

Publishing a GitHub Release automatically builds the released commit and pushes
versioned application and operations images to GitHub Container Registry. The
maintainer still controls pushes, tags, and the hosted release itself.

The MCP server image is released from
[nojusmorkunas/conatus-mcp](https://github.com/nojusmorkunas/conatus-mcp) on its
own schedule and is not built here.

## Candidate: 0.2.0-beta.1

### 1. Freeze and verify

1. Create a release branch from the intended commit.
2. Confirm `TODO.md` and the 0.2.0 section of `CHANGELOG.md` match the diff.
3. Install from the lockfile with `npm ci`.
4. Run `npm test`, `npm run lint`, `npm run build`, and `npm run test:e2e`.
5. Start from an empty database, apply every migration, and run the core browser flow.
6. Run `scripts/upgrade-check.sh --all`, or confirm the `Verify upgrades` workflow is green on the release branch. Every published release must upgrade cleanly, not only the previous one.
7. Exercise Todoist API and ZIP imports using non-production Todoist fixtures.
8. Build the Docker Compose stack and verify health, login, a scheduled reminder, object upload, backup, and restore.

### 2. Security and operations check

1. Confirm production secrets are not present in tracked files or build output.
2. Rotate any credential used in release testing.
3. Verify API tokens respect scope, expiry, revocation, and idempotency behavior.
4. Verify MCP HTTP OAuth discovery, PKCE, refresh rotation, and static-token fallback in the target topology.
5. Confirm SMTP, storage, job worker, proxy headers, public base URL, and health checks are configured.

### 3. Publish (maintainer only)

1. Update versions and lockfiles to `0.2.0-beta.1`; commit the release artifacts.
2. Tag the verified commit `v0.2.0-beta.1` and push the branch and tag.
3. Create a prerelease using the matching `CHANGELOG.md` section, migration notes, and rollback instructions. Publishing it triggers `.github/workflows/release-images.yml`.
4. Confirm the workflow pushed matching `<version>`, `<version>-ops`, and MCP `<version>` images, and record their digests. Prereleases do not move `latest`.
5. If publishing the MCP npm package, verify the package scope, run `npm pack --dry-run`, then publish with the beta dist-tag.
6. Deploy the exact version to the beta environment, rerun health and core-flow smoke checks, then invite the beta cohort.

### 4. Observe and roll back

Monitor error rate, failed jobs, webhook failures, import errors, latency, and
database/storage growth. If data integrity or authentication regresses, stop the
rollout, preserve logs, restore the pre-release database backup if required, and
redeploy the prior immutable image. Do not reverse a migration against the only
copy of user data.
