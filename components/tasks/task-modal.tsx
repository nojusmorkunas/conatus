...
export function TaskModal({ task, labels, members = [], currentUserId, today, dateFormat, onClose, onChanged, onDelete, onPrev, onNext }: {
  ...
  const avatar = currentMember?.email.charAt(0).toUpperCase() ?? "•";
    ...
    <div role="dialog" aria-modal="true" className="mx-auto flex h-full max-h-[82vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl border bg-background shadow-xl">
      ...
      <div className="flex flex-1 flex-col overflow-y-auto md:flex-row md:overflow-hidden">
        ...
        <aside className="w-full shrink-0 border-t bg-muted/20 px-4 py-2 md:w-64 md:overflow-y-auto md:border-t-0 md:border-l">
          ...
          {members.length > 1 && <RailRow label="Assignee"><Select value={task.assigneeId ?? undefined} onValueChange={(value) => void patch({ assigneeId: value })}><SelectTrigger size="sm" aria-label="Assignee"><SelectValue placeholder="Choose member">{task.assigneeId ? members.find((member) => member.id === task.assigneeId)?.email : null}</SelectValue></SelectTrigger><SelectContent>{members.map((member) => <SelectItem key={member.id} value={member.id}>{member.email}</SelectItem>)}</SelectContent></Select>{task.assigneeId && <Button variant="ghost" size="sm" onClick={() => void patch({ assigneeId: null })}>Unassign</Button>}</RailRow>}
