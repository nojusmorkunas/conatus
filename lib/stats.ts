...
// A day counts towards a streak as soon as one task is completed on it.
export function computeStats(
  completionDates: string[],
  opts: { today: string },
): { currentStreak: number; longestStreak: number } {
  const active = new Set(completionDates);

  let currentStreak = 0;
  let currentDate = active.has(opts.today) ? opts.today : addDays(opts.today, -1);
  while (active.has(currentDate)) {
    currentStreak += 1;
